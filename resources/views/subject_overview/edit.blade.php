@extends('layouts.app', [
    'title' => __('subject_overview Management'),
...
@section('content')
    @component('layouts.headers.auth')
        @component('layouts.headers.breadcrumbs')
            ...
            <li class="breadcrumb-item active" aria-current="page">{{ __('Edit Subject Overview') }}</li>
        @endcomponent
    @endcomponent
        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('subject_overview.update', $subjectOverview->id) }}" autocomplete="off"
                            enctype="multipart/form-data">
                            @csrf
                            {{ method_field('PATCH') }}

                            <h6 class="heading-small text-muted mb-4">{{ __('subject_overview information') }}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group">
                                    ...
                                    <select name="subject_id" id="" class="form-control">
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}" {{ $subjectOverview->subject_id==$subject->id?'selected':'' }}>{{ $subject->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="overview" id="input-subject_overview" rows="5" class="form-control" placeholder="Enter subject_overview">{{ $subjectOverview->overview }}</textarea>
